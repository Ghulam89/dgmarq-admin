...
const FlashDeals = () => {
    ...
    const updateStatus = (id, newStatus) => {
        axios
            .put(`${Base_url}/flashDeals/approve/${id}`, { status: newStatus })
            .then((res) => {
                if (res.status === 200) {
                                                ...
                                                <tr key={item._id} className="bg-white border-t rounded-md">
                                                    ...
                                                    <td className="align-middle text-sm font-normal px-6 py-4 whitespace-nowrap">
                                                        <div className="flex justify-center items-center gap-3">
                                                            <div>
                                                                <Link to={`/flash-deals-product/${item?._id}`} className="flex gap-2 items-center border-primary border px-3 hover:bg-primary hover:text-white text-primary py-1.5 rounded-sm">
                                                                    <FaPlus /> Add Product
                                                                </Link>
                                                            </div>
